...
function createCard(containerId, title, count) {
  const card = document.createElement('div'); card.className = 'card'; card.id = 'card-' + containerId;
  ...
  const body = document.createElement('div'); body.className = 'card-body'; body.id = containerId;
...
function cardSuccess(containerId, html) { const el = document.getElementById(containerId); if (!el) return; el.innerHTML = html; }
function cardEmpty(containerId, message) { const el = document.getElementById(containerId); if (!el) return; el.innerHTML = '<div class="card-empty">' + escapeHtml(message || 'No data') + '</div>'; }
function cardError(containerId, message, retryFn) {
  const el = document.getElementById(containerId); if (!el) return;
  el.innerHTML = '<div class="card-error"><p>' + escapeHtml(message) + '</p>' + (retryFn ? '<button class="btn-retry" onclick="' + retryFn + '()">Retry</button>' : '') + '</div>';
}
function updateCardCount(containerId, count) {
  const card = document.getElementById('card-' + containerId); if (!card) return;
  const badge = card.querySelector('.card-count'); if (badge) badge.textContent = count;
}
...
function statusBadge(status) {
  const map = { now: 'badge-error', in_review: 'badge-warning', next: 'badge-info', new: 'badge-muted', waiting: 'badge-warning', in_queue: 'badge-info', pending: 'badge-warning', healthy: 'badge-success', critical: 'badge-error', stale: 'badge-warning', warning: 'badge-warning' };
...
function renderSortableTable(containerId, columns, rows, opts) {
  ...
  let currentPage = 0, sortCol = opts.defaultSort || null, sortDir = opts.defaultSortDir || 'asc', filterText = '';
  function render() {
    let filtered = rows;
    if (filterText) { const q = filterText.toLowerCase(); filtered = rows.filter(r => columns.some(c => String(r[c.key] || '').toLowerCase().includes(q))); }
    if (sortCol) {
      filtered = [...filtered].sort((a, b) => {
        let va = a[sortCol], vb = b[sortCol]; if (va == null) va = ''; if (vb == null) vb = '';
    ...
    const start = currentPage * pageSize, pageRows = filtered.slice(start, start + pageSize);
    let html = '';
    if (rows.length > 10) html += '<div class="filter-bar"><input type="text" placeholder="Search..." value="' + escapeHtml(filterText) + '" onkeyup="window._tblFilter_' + containerId + '(this.value)"></div>';
    html += '<table class="data-table"><thead><tr>';
    columns.forEach(c => { const isSorted = sortCol === c.key; const arrow = isSorted ? (sortDir === 'asc' ? ' \u25B2' : ' \u25BC') : ' \u25B4'; html += '<th class="' + (isSorted ? 'sorted' : '') + '" onclick="window._tblSort_' + containerId + '(\'' + c.key + '\')">' + escapeHtml(c.label) + '<span class="sort-icon">' + arrow + '</span></th>'; });
    html += '</tr></thead><tbody>';
    if (pageRows.length === 0) html += '<tr><td colspan="' + columns.length + '" style="text-align:center;color:var(--text-dim);padding:1rem">No results</td></tr>';
    else pageRows.forEach(r => { html += '<tr>'; columns.forEach(c => { const val = c.render ? c.render(r) : escapeHtml(r[c.key]); html += '<td>' + val + '</td>'; }); html += '</tr>'; });
    html += '</tbody></table>';
    if (total > pageSize) {
      ...
      html += '<button ' + (currentPage >= pages - 1 ? 'disabled' : '') + ' onclick="window._tblPage_' + containerId + '(' + (currentPage + 1) + ')">Next</button></div></div>';
    }
    cardSuccess(containerId, html);
  }
  window['_tblSort_' + containerId] = function(col) { if (sortCol === col) sortDir = sortDir === 'asc' ? 'desc' : 'asc'; else { sortCol = col; sortDir = 'asc'; } render(); };
...
// Shared Ops Helpers (used by ops-t*.js)
...
function opsSection(label, innerHtml) {
  if (!innerHtml) return '';
  return '<div class="ops-section" style="padding:0 1.5rem 0.75rem"><div class="ops-section-header" style="padding:0.5rem 0;font-size:0.85rem;font-weight:600;color:var(--text-muted)">' + escapeHtml(label) + '</div><div class="card" style="overflow:hidden"><div class="card-body">' + innerHtml + '</div></div></div>';
}
