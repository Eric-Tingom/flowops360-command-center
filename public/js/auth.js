...
const SUPABASE_URL = 'https://bbsldtgusmjpulohxzpa.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJic2xkdGd1c21qcHVsb2h4enBhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzAwNTAyNzQsImV4cCI6MjA4NTYyNjI3NH0.wnRJpsDrXn2dRqQQsGVzToAeauxZgboPiXPxRc-5Xq8';
...
async function doLogin() {
  ...
  try {
    ...
    if (error) {
      failedLogins++;
      errEl.textContent = error.message && error.message.includes('Invalid login credentials') ? 'Invalid email or password.' : failedLogins >= 3 ? 'Authentication may not be configured. Contact admin.' : error.message || 'Login failed.';
      errEl.style.display = 'block'; btn.disabled = false; btn.textContent = 'Sign In'; return;
    }
    showDashboard(data.user);
  } catch (e) { errEl.textContent = 'Unable to connect.'; errEl.style.display = 'block'; btn.disabled = false; btn.textContent = 'Sign In'; }
}
...
function renderNav() {
  ...
  PILLARS.forEach(p => {
    ...
    tab.onclick = () => navigateTo(p.id);
    nav.appendChild(tab);
  });
}
function navigateTo(pillarId) {
  activePillar = pillarId; renderNav();
  const content = document.getElementById('pillar-content');
  content.innerHTML = '';
  const pillar = PILLARS.find(p => p.id === pillarId);
  if (pillar && pillar.loader) {
    const fn = typeof pillar.loader === 'string' ? window[pillar.loader] : pillar.loader;
    if (typeof fn === 'function') fn(content);
    else content.innerHTML = '<div class="placeholder-pillar"><h2>' + escapeHtml(pillar.label) + '</h2><p>Loader not found: ' + escapeHtml(pillar.loader) + '</p></div>';
  } else {
...
async function handleCardError(containerId, rpcName, error, retryFn) {
  ...
  logError(containerId, rpcName, error?.code || 'UNKNOWN', error?.message || 'Unknown error');
  cardError(containerId, error?.message || 'Failed to load data', retryFn);
}
