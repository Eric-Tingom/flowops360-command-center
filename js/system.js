...
// Cron Jobs, HTTP Log, Agent Registry sub-tabs
  ...
  try {
    ...
    function render() {
      ...
      if (filtered.length === 0) {
        ...
        filtered.forEach(l => {
          const statusHtml = l.timed_out
            ? '<span class="badge badge-error">Timeout</span>'
            : l.status_code
              ? (l.status_code === 200 ? '<span class="badge badge-success">200</span>' : '<span class="badge badge-error">' + l.status_code + '</span>')
              : '<span class="badge badge-muted">\u2014</span>';
          const errMsg = l.error_msg ? escapeHtml(l.error_msg.substring(0, 80)) : '\u2014';
          html += '<tr><td>' + l.id + '</td><td>' + statusHtml + '</td><td>' + errMsg + '</td><td style="white-space:nowrap">' + formatDate(l.created) + '</td></tr>';
        });
      }
      html += '</tbody></table>';
      container.innerHTML = html;
...
async function loadAgentRegistry(container) {
  container.innerHTML = '<div style="padding:1rem"><div class="shimmer" style="width:80%"></div><div class="shimmer" style="width:60%"></div></div>';
  try {
    const { data, error } = await sb.from('assistants')
      .select('assistant_id, assistant_name, agent_type, brain, is_active, category, functional_domains, last_active_at')
      .eq('is_active', true)
    ...
    function render() {
      ...
      if (searchText) {
        ...
        filtered = filtered.filter(a =>
          (a.assistant_name || '').toLowerCase().includes(q) ||
          (a.assistant_id || '').toLowerCase().includes(q) ||
      ...
      html += '<input type="text" placeholder="Search agents..." value="' + escapeHtml(searchText) + '" oninput="window._agentSearch(this.value)">';
      html += '<select onchange="window._agentTypeFilter(this.value)"><option value="">All types</option>';
      types.forEach(t => {
        html += '<option value="' + escapeHtml(t) + '"' + (typeFilter === t ? ' selected' : '') + '>' + escapeHtml(t) + '</option>';
      });
      html += '</select>';
      html += '<span style="color:var(--text-muted);font-size:0.8rem">' + filtered.length + ' agents</span>';
      html += '</div>';
