...
// 9 card loaders using existing standup RPCs
...
function loadOperations(container) {
  ...
  cards.forEach(c => {
    grid.appendChild(createCard(c.id, c.title));
    c.loader();
  });
...
async function loadOverdueTickets() {
  ...
  try {
    ...
    data.by_client.forEach(c => {
      const t = c.tickets && c.tickets[0];
      html += '<tr><td>' + clientBadge(c.client_name) + '</td><td>' + (c.count || c.tickets?.length || 0) + '</td><td class="truncate">' + escapeHtml(t?.subject || '\u2014') + '</td><td>' + linkIcon(t?.hubspot_ticket_url) + '</td></tr>';
    });
    html += '</tbody></table>';
...
async function loadOpenTickets() {
  ...
  try {
    ...
    let warn = '';
    const displayTickets = tickets.length > 500 ? (warn = '<div class="warning-banner">Showing first 500 of ' + tickets.length + ' tickets</div>', tickets.slice(0, 500)) : tickets;
    const columns = [
    ...
    if (warn) el.innerHTML = warn;
    renderSortableTable('open-tickets', columns, displayTickets, { defaultSort: 'last_modified', defaultSortDir: 'desc' });
